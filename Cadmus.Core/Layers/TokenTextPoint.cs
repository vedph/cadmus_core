...
{
    /// <summary>
    /// Token-based text point: this is the point building up token-based text location
    /// (<see cref="TokenTextLocation"/>).
    /// </summary>
    /// <remarks>
    /// <para>Token-based coordinates include two main components, <see cref="Y"/> and <see cref="X"/>; 
    /// <c>Y</c> represents the atomic text block (e.g. a line) and <c>X</c> the token ordinal 
    /// in this block.</para>
	/// <para>Further, optionally a text coordinate can refer to just a portion of a token, defined
	/// by the ordinal of its first character (<see cref="At"/>) and the number of characters to 
	/// include starting from it (<see cref="Run"/>); for instance, when mapping a translation
	/// we might require to map the token "and" in "and he" from "isque" to just the last
	/// 3 characters ("que") of the unique token "isque". In this case we can set <c>At</c>=3
	/// and <c>Run</c>=3.</para> 
	/// </remarks>
    public sealed class TokenTextPoint : ITextPoint
    {
        ...
        /// </summary>
        /// <remarks>This property is used together with <see cref="Run"/> to define a region
        /// in the token referenced by <see cref="X"/>. For instance, when mapping a translation
        /// we might require to map the token "and" in "and he" from "isque" to just the last
        /// 3 characters ("que") of the unique token "isque". In this case we can set <c>At</c>=3
        /// and <c>Run</c>=3.</remarks>
        /// <value>ordinal of token character or 0 if not used.</value>
        /// <exception cref="ArgumentOutOfRangeException">attempt to set to value &lt; 0</exception>
        public short At { get; set; }

        /// <summary>
        /// Optional count of characters run in token.
        /// </summary>
        /// <remarks>This property is used together with <see cref="At"/> to define a region
        /// in the token referenced by <see cref="X"/>. For instance, when mapping a translation
        /// we might require to map the token "and" in "and he" from "isque" to just the last
        /// 3 characters ("que") of the unique token "isque". In this case we can set <c>At</c>=3
        /// and <c>Run</c>=3.</remarks>
        /// <value>count of characters to run from <see cref="At"/>, or 0 if not used.</value>
        /// <exception cref="ArgumentOutOfRangeException">attempt to set to value &lt; 0</exception>
        ...
        /// <exception cref="ArgumentOutOfRangeException">invalid at/run value</exception>
        ...
        {
            if (!(point is TokenTextPoint p)) throw new ArgumentNullException(nameof(point));
        ...
        /// Parse coords text.
        /// </summary>
        /// <remarks>Coords text is in the form <c>Y.X</c> or <c>Y.X@AxR</c> or <c>Y.X@A</c> (in this case
        ...
        {
            if (!(other is TokenTextPoint o)) throw new ArgumentNullException(nameof(other));
        ...
        public int CompareTo(object obj)
        {
            ...
            if (!(obj is TokenTextPoint))
                throw new ArgumentException($"Object must be of type {nameof(TokenTextPoint)}");
