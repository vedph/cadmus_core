...
{
    /// <summary>
    /// MongoDB based repository for Cadmus data. Tag: <c>cadmus-repository.mongo</c>.
    /// </summary>
    /// <remarks>The Mongo database has the following collections:
    /// <list type="bullet">
    /// <item>
    ///     <term>flags</term>
    ///     <description>collection of <see cref="StoredFlagDefinition"/>'s.</description>
    /// </item>
    /// <item>
    ///     <term>facets</term>
    ...
    /// <item>
    ///     <term>parts</term>
    ///     <description>collection of <see cref="BsonDocument"/>'s representing objects implementing
    /// <see cref="IPart"/>, whose standard properties are <c>Id</c>, <c>ItemId</c>, <c>TypeId</c>, 
    /// <c>RoleId</c>, <c>TimeModified</c>, <c>UserId</c>. Each part type then adds its own
    /// properties.</description>
    /// </item>
    /// <item>
    ///     <term>history-parts</term>
    ///     <description>collection of <see cref="HistoryWrapper{BsonDocument}"/>'s: its properties
    /// are <c>Id</c>, <c>TimeModified</c>, <c>UserId</c>, <c>Status</c>, <c>Content</c>, which
    /// represents the objects implementing <see cref="IPart"/>.
    /// </description>
    /// </item>
    /// </list>
    /// <para>As for parts, their models vary according to the part type, and part types are
    /// not known before runtime, as they come from plugins. When reading an item with its
    /// parts (<see cref="GetItem"/>), an instance of <see cref="IPartTypeProvider"/> is
    /// used to get the C# type for each part, and then this type information is used
    /// to instantiate the corresponding object during BSON deserialization. When adding
    /// an item (<see cref="AddItem"/>), no parts are added; each part is added separately
    /// using <see cref="AddPart"/>, which just stores the part object as a BSON document.
    /// </para>
    /// </remarks>
        ...
        IConfigurable<MongoCadmusRepositoryOptions>
    {
        ...
        {
            _partTypeProvider = partTypeProvider ?? throw new ArgumentNullException(nameof(partTypeProvider));
        ...
        public void AddFlagDefinition(IFlagDefinition definition)
        {
            ...
            collection.ReplaceOne(f => f.Id.Equals(definition.Id), new StoredFlagDefinition(definition),
        ...
        public void AddFacet(IFacet facet)
        {
            ...
            collection.ReplaceOne(f => f.Id.Equals(facet.Id), new StoredItemFacet(facet),
                new UpdateOptions {IsUpsert = true});
        }
        ...
        public TagSet GetTagSet(string id)
        {
            ...
            StoredTagSet stored = collection.AsQueryable().FirstOrDefault(set => set.Id == id);
            return stored == null
                ? null
                : new TagSet
                {
        ...
        public void AddTagSet(TagSet set)
        {
            ...
            collection.ReplaceOne(old => old.Id.Equals(set.Id), new StoredTagSet(set),
                new UpdateOptions {IsUpsert = true});
        }
        ...
        public PagedData<IItemInfo> GetItemsPage(ItemFilter filter)
        {
            ...
            IMongoDatabase db = _client.GetDatabase(_options.DatabaseName);
            IMongoCollection<StoredItem> collection = db.GetCollection<StoredItem>(StoredItem.COLLECTION);
        ...
        public IItem GetItem(string id, bool includeParts = true)
        {
            ...
            StoredItem stored = collection.Find(i => i.Id.Equals(id)).FirstOrDefault();
            ...
            if (includeParts)
            {
                IMongoCollection<BsonDocument> parts = db.GetCollection<BsonDocument>(COLL_PARTS);
        ...
        /// </summary>
        /// <remarks>Note that the item's parts are ignored. If you want to add parts, use
        /// the <see cref="AddPart"/> method.</remarks>
        ...
        public void AddItem(IItem item, bool history = true)
        {
            ...
            IMongoDatabase db = _client.GetDatabase(_options.DatabaseName);
            IMongoCollection<StoredItem> items = db.GetCollection<StoredItem>(StoredItem.COLLECTION);
            ...
            if (history)
            {
                ...
                StoredItem old = items.Find(i => i.Id.Equals(item.Id)).FirstOrDefault();
        ...
        /// Deletes the item.
        /// </summary>
        /// <remarks>Note that deleting an item does triggers the deletion of all its parts.</remarks>
        ...
        public void DeleteItem(string id, string userId, bool history = true)
        {
            ...
            // delete the item's parts (one at a time as we need to store them into history, too)
            IMongoCollection<BsonDocument> partsCollection = db.GetCollection<BsonDocument>(COLL_PARTS);
            ...
            // store the item being deleted into history, as deleted now by the specified user
            if (history)
            {
        ...
        public void SetItemFlags(string id, int flags)
        {
            ...
            items.UpdateOne(i => i.Id.Equals(id), Builders<StoredItem>.Update.Set("flags", flags));
        }

        private static IHistoryItemInfo CreateHistoryItemInfo(StoredHistoryItem item)
        {
        ...
        /// <exception cref="ArgumentNullException">null filter</exception>
        public PagedData<IHistoryItemInfo> GetHistoryItemsPage(HistoryItemFilter filter)
        {
            if (filter == null) throw new ArgumentNullException(nameof(filter));
        ...
        public IHistoryItem GetHistoryItem(string id)
        {
            ...
            StoredHistoryItem item = items.Find(i => i.Id.Equals(id)).FirstOrDefault();
        ...
        //private string GetPartFilterJson(PartFilter filter)
        //{
        ...
        //    {
        //        sb.Append("{\"ItemId\":");
        //        sb.Append(filter.ItemIds.Length == 1
        //            ? $"\"{filter.ItemIds[0]}\"" 
        //            : "{$in:[" + String.Join(",", filter.ItemIds) + "]}");
        //        sb.Append('}');
        //    } //eif

        //    if (filter.TypeId != null)
        //        sb.Append(",$and: [{\"TypeId\": \""+ filter.TypeId + "\"}]");

        //    if (filter.RoleId != null)
        //        sb.Append(",$and: [{\"RoleId\": \"" + filter.RoleId + "\"}]");

        //    if (filter.UserId != null)
        //        sb.Append(",$and: [{\"UserId\": \"" + filter.UserId + "\"}]");

        //    if (filter.MinModified.HasValue)
        //        sb.Append(",$and: [{\"TimeModified\": {$gte: ISODate(\"" +
        //                  MongoHelper.GetIsoDate(filter.MinModified.Value) +
        //                  "\")}}]");

        //    if (filter.MaxModified.HasValue)
        //        sb.Append(",$and: [{\"TimeModified\": {$lte: ISODate(\"" +
        //                  MongoHelper.GetIsoDate(filter.MinModified.Value) +
        //                  "\")}}]");
        ...
        public PagedData<IPartInfo> GetPartsPage(PartFilter filter)
        {
            ...
            if (filter.MinModified.HasValue)
                parts = parts.Where(d => d["timeModified"].ToUniversalTime() >= filter.MinModified.Value);

            if (filter.MaxModified.HasValue)
                parts = parts.Where(d => d["timeModified"].ToUniversalTime() <= filter.MaxModified.Value);

            int total = parts.Count();
            if (total == 0) return new PagedData<IPartInfo>(0, new List<IPartInfo>());
            ...
            return new PagedData<IPartInfo>(total, results.Select(CreatePartInfo).ToList());
        }
        ...
        /// <exception cref="ArgumentOutOfRangeException">empty item IDs array</exception>
        public IList<IPart> GetItemParts(string[] itemIds, string typeId = null, string roleId = null)
        {
            if (itemIds == null) throw new ArgumentNullException(nameof(itemIds));
            if (itemIds.Length == 0) throw new ArgumentOutOfRangeException(nameof(itemIds));
        ...
        /// <param name="history">if set to <c>true</c>, the history should be affected.</param>
        /// <exception cref="ArgumentNullException">null part or user ID</exception>
        public void AddPart(IPart part, bool history = true)
        {
            ...
            if (history)
            {
                BsonDocument old = parts.Find(d => d["_id"].Equals(part.Id)).FirstOrDefault();
                BsonDocument historyPart = new BsonDocument
                {
                    ...
                    new BsonElement("status", old == null ? EditStatus.Created : EditStatus.Updated),
        ...
        public void AddPartJson(string json, bool history = true)
        {
            ...
            if (history)
            {
                BsonDocument old = parts.Find(d => d["_id"].Equals(part["_id"])).FirstOrDefault();
                BsonDocument historyPart = new BsonDocument
                {
                    new BsonElement("_id", Guid.NewGuid().ToString("N")),
                    new BsonElement("timeModified", part["timeModified"]),
        ...
        /// <param name="history">if set to <c>true</c>, the history should be affected.</param>
        /// <exception cref="ArgumentNullException">null part ID or user ID</exception>
        public void DeletePart(string id, string userId, bool history = true)
        {
            ...
            BsonDocument part = parts.Find(d => d["_id"].Equals(id)).FirstOrDefault();
            if (part == null) return;
        ...
        /// <param name="filter">The filter.</param>
        /// <returns>history items page</returns>
        public PagedData<IHistoryPartInfo> GetHistoryPartsPage(HistoryPartFilter filter)
        {
            if (filter == null) throw new ArgumentNullException(nameof(filter));

            IMongoDatabase db = _client.GetDatabase(_options.DatabaseName);
            IMongoCollection<BsonDocument> collection = db.GetCollection<BsonDocument>(COLL_HISTORYPARTS);
            ...
            if (filter.ItemIds != null && filter.ItemIds.Length > 0)
            {
                ...
                {
                    List<BsonValue> aValues = filter.ItemIds.Select(BsonValue.Create).ToList();
                    parts = parts.Where(d => aValues.Contains(d["content"]["itemId"]));
                }
            }

            if (filter.ReferenceId != null)
                parts = parts.Where(d => d["content"]["_id"].Equals(filter.ReferenceId));

            if (filter.Status.HasValue)
                parts = parts.Where(d => d["status"].Equals(filter.Status.Value));
            ...
            int total = parts.Count();
            if (total == 0) return new PagedData<IHistoryPartInfo>(0, new List<IHistoryPartInfo>());
        ...
        /// <returns>part or null if not found</returns>
        /// <exception cref="ArgumentNullException">null ID</exception>
        public IHistoryPart<T> GetHistoryPart<T>(string id) where T : class, IPart
        {
            if (id == null) throw new ArgumentNullException(nameof(id));
        ...
        public void AddHistoryPart<T>(IHistoryPart<T> part) where T : class,IPart
        {
            if (part == null) throw new ArgumentNullException(nameof(part));
    ...
    {
        /// <summary>
        /// Gets or sets the MongoDB connection string template, where <c>{0}</c> is a placeholder
        /// for the database name.
        /// </summary>
        /// <value>The default value is <c>mongodb://localhost:27017/{0}</c>.</value>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="MongoCadmusRepositoryOptions"/> class.
        /// </summary>
