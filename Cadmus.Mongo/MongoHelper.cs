...
namespace Cadmus.Mongo
{
    ...
    public static class MongoHelper
    {
        ...
        /// <summary>
        /// Gets the specified date in UTC ISO format (yyyy-mm-ddThh:mm:ss.mZ).
        /// </summary>
        /// <param name="dt">The dt.</param>
        /// <returns>string with the date in ISO format</returns>
        public static string GetIsoDate(DateTime dt)
        {
            DateTime utc = dt.ToUniversalTime();
            return $"{utc.Year:0000}-{utc.Month:00}-{utc.Day:00}T" +
                   $"{utc.Hour:00}:{utc.Minute:00}:{utc.Second:00}.{utc.Millisecond}Z";
        }
    }
}
